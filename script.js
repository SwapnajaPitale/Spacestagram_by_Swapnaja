...
let response = await sendHTTPRequest("GET", 'https://api.nasa.gov/planetary/apod?api_key=DXqVFrcdSgOsOyCmeGDspbGnLHqrXI4U2EqAfZNG');
...
if(mediaType==='image'){
  document.getElementById('cardImg').innerHTML = `<img class="card-img-top" src=${photoOfToday} alt=${title} id='photoOfToday'>`;
} else if (mediaType === 'video'){
  document.getElementById('cardImg').innerHTML = `<iframe class="card-img-top" src=${photoOfToday} alt=${title} id='photoOfToday'></iframe>`;
} else{
  document.getElementById('cardImg').innerHTML = `<p>Unsupported media type</p>`;
}
...
`<a href=${photoOfToday} target='_blank'>${photoOfToday}</a>`
}
getQuotes();